...
def label_video(detection_graph, category_index, cap, feather_fname, base_name,
                nb_frames=100000000, start_frame=0, config_proto=None):
    all_rows = []
    crop = get_crop_dict()[base_name]
    with detection_graph.as_default():
        with tf.Session(graph=detection_graph, config=config_proto) as sess:
            for i in tqdm.trange(nb_frames):
                ...
                for j, box in enumerate(boxes):
                    ...
                    ymin = ymin * im.shape[0] + crop[1]
                    ymax = ymax * im.shape[0] + crop[1]
                    xmin = xmin * im.shape[1] + crop[0]
                    xmax = xmax * im.shape[1] + crop[0]
...
def main():
    ...
    label_video(detection_graph, category_index, cap, args.feather_fname, args.base_name,
